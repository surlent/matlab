function [Result,tp] = BPtrainIndex(stockcode,Flag,unit)
%% 读取训练数据,并修剪包含0值的矩阵.
%Flag=1,只做学习不回测
%Flag=2,只回测不学习
%Flag=3,学习并回测
...
if Flag==1 || Flag==3
    ...
    %% 保存训练网络
    save( ['.\DataBase\Net\Index\' stockcode '_net.mat'],'net', '-v7.3');%文件用来保存net
    save( ['.\DataBase\Net\Index\' stockcode '_tp.mat'],'tp', '-v7.3');%文件用来保存回测结果
    if Flag==1
        Result={stockcode,0,0,0,0,0,0,0};
    end
end
