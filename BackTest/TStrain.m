function [Result] = TStrain(stockcode,Flag)
...
if Flag==1 || Flag==3
    ...
    %% 保存训练网络
    save( ['.\DataBase\Net\TSStock\' stockcode '_net.mat'],'net', '-v7.3');%文件用来保存回测结果
    if Flag==1
        Result={stockcode,0,0,0,0,0,0,0};
    end
end
...
if Flag==2 || Flag==3
    ...
    if Flag==2
        try
            ...
            try
                [Result]=TStrain(stockcode,1);
                disp(['无神经网络文件,尝试学习生成' stockcode '神经网络'])
                load(['.\DataBase\Net\Stock\' stockcode '_net.mat']);
            catch
                disp('样本数据不足导致输入输出维度过低,退出本条神经网络训练')
                return
    ...
    for i=2:Len                   %全都是有效数据从第二天开始
        signalBuy= ForCast(i-1)>0 && ROE(i)>5;
...
if Flag==4
    ...
    TestData(any(TestData==0,2),:)=[];
    if isempty(TestData)
        disp(['代码' stockcode '测试数据不足导致无法回测,退出本条预测'])
