function [Result,DetailProcess] = BT_RSImean( Slip,StartMoney,Fee,Date,Scode,MeanCost5,MeanCost10,MeanCost20,MeanCost40,MeanCost80,MeanCost160,MeanCost320,RSIValue,Price,TOR,Period)
...
for i=100:Len                   %过滤掉新股阶段
    if RSIValue(i-1)>50 && MeanCost5(i-2)<MeanCost40(i-2) && MeanCost5(i-1)>MeanCost40(i-1) && Position(i-1)==0 %入场点较为苛刻
       Position(i)=1;
    ...
    elseif (RSIValue(i-1)<50 && MeanCost5(i-2)>MeanCost20(i-2) && MeanCost5(i-1)<MeanCost20(i-1) && Position(i-1)==1) || (MeanCost5(i-1)<TradePrice(i-1,1)*0.9 && Position(i-1)==1) || (Price(i-1)>TradePrice(i-1,1)*1.5 && Position(i-1)==1) %或者符号后方为止损,止盈条件
       Position(i)=0;
