function [Result,DetailProcess] = BT_macd(Slip,StartMoney,Fee,Price,DIF,DEA,Scode,Date)
%%回测某代码数据的
Len=length(Price);
if Len<=100
...
for i=100:Len                   %过滤掉新股阶段
    if DIF(i-1)>DEA(i-1) && DIF(i-2)<=DEA(i-2) && Position(i-1)==0
       Position(i)=1;
    ...
    elseif DIF(i-1)<DEA(i-1) && DIF(i-2)>=DEA(i-2) && Position(i-1)==1
       Position(i)=0;
