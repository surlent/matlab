...
if Mod==1
    ...
    if Flag~=1 && Flag~=4                   %flag2.3需要汇总result数据,1没有result反馈,4的result反馈为次日策略建议,格式不同
        ...
        save( '.\DataBase\BackTestResult\AI\Result.mat','result', '-v7.3');%文件用来保存回测结果
        save( '.\DataBase\BackTestResult\AI\Mean.mat','Mean', '-v7.3');%文件用来保存汇总
    end
    if Flag==4
        disp('汇总预测结果生成xls文件')
        ...
        xlswrite(['.\BackTest\AIForcast\SubTotal' LastDate ],{'代码','最后预测日期','建议','预计涨跌幅%','回测期望年化收益率','回测期望夏普比率','最大回撤','历史胜率','历史交易次数(越多越好)','交易长度(越多越好)'},'回测结果','A1');
        xlswrite(['.\BackTest\AIForcast\SubTotal' LastDate ],result,'回测结果','A2');
    end
end
...
if Mod==2
    ...
    if Flag~=1 && Flag~=4
        ...
        temp=num2cell(temp);                %计算平均值
        Mean=[{'平均总收益'},{'平均年化收益率'},{'平均夏普比率'},{'平均最大回撤'},{'平均胜率'},{'平均交易次数'},{'平均交易长度'};temp];
        save( '.\DataBase\BackTestResult\AI_Index\Result.mat','result', '-v7.3');%文件用来保存回测结果
        save( '.\DataBase\BackTestResult\AI_Index\Mean.mat','Mean', '-v7.3');%文件用来保存汇总
    end
    if Flag==4
        disp('汇总预测结果生成xls文件')
        ...
        LastDate=num2str(max(cell2mat(result(:,2))));
        xlswrite(['.\BackTest\AIForcast_Index\SubTotal' LastDate ],{'代码','最后预测日期','建议','预计涨跌幅','回测期望年化收益率','回测期望夏普比率','最大回撤','历史胜率(可能存在过拟合)','历史交易次数(越多越好)','交易长度(越多越好)'},'回测结果','A1');
        xlswrite(['.\BackTest\AIForcast_Index\SubTotal' LastDate ],result,'回测结果','A2');
    end
    clear
end
