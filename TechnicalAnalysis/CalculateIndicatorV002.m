function CalculateIndicatorV002(Scode,FileString,Flag,IndexIndicatorsSH,IndexIndicatorsSZ)
%%计算指标函数
%7月1日更新,320日线取消,这个指标不适合10日预测
%6月2日更新
%增加多周期的boll和obv指标作为参考数据
%5月28日更新
...
%     end
    ...
    if Len>=Length7
        ...
        StockIndicators(Length7:end,22)=Price(Length7:end)./UpperLine(Length7:end)-1;
        clear UpperLine;
        StockIndicators(Length7:end,23)=Price(Length7:end)./MiddleLine(Length7:end)-1;
        clear MiddleLine;
        StockIndicators(Length7:end,24)=Price(Length7:end)./LowerLine(Length7:end)-1;
        clear LowerLine;
    end
